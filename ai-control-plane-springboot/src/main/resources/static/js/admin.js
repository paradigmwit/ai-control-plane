(function () {
    const { formatJson, requestJson, setStatus } = window.appShared;
    ...
    const endpointGroups = [
        {
            ...
            endpoints: [
                {
                    id: 'user-workflow-execute',
                    name: 'Execute currency conversion workflow',
                    method: 'POST',
                    path: '/api/user/orchestration/workflows/currency-conversion/execute',
                    body: { prompt: 'Convert 100 USD to EUR' }
                },
                {
                    id: 'user-plan-create',
                    name: 'Create user plan',
                    method: 'POST',
                    path: '/api/user/orchestration/plan',
                    body: { taskSummary: 'Plan a multi-step task', metadata: 'manual request from admin console' }
                },
                {
                    id: 'user-plan-execute',
                    name: 'Execute user plan',
                    method: 'POST',
                    path: '/api/user/orchestration/execute/{planId}',
                    pathParams: [{ key: 'planId', value: 'workflow.currency-conversion.v1' }]
                },
                {
                    id: 'user-execution-status',
                    name: 'Get execution status',
                    method: 'GET',
                    path: '/api/user/orchestration/executions/{instanceId}',
                    pathParams: [{ key: 'instanceId', value: 'instance-123' }]
                },
                {
                    id: 'user-execution-steps',
                    name: 'List execution steps',
                    method: 'GET',
                    path: '/api/user/orchestration/executions/{instanceId}/steps',
                    pathParams: [{ key: 'instanceId', value: 'instance-123' }]
                },
                {
                    id: 'user-step-complete',
                    name: 'Mark step complete',
                    method: 'POST',
                    path: '/api/user/orchestration/executions/{instanceId}/steps/{stepId}/complete',
                    pathParams: [
                        { key: 'instanceId', value: 'instance-123' },
                        { key: 'stepId', value: 'currency-conversion.convert-amount' }
                    ]
                },
                {
                    id: 'user-profiles',
    ...
    groupsContainer.querySelector('[data-endpoint-id="user-workflow-execute"]')?.classList.add('active');
    ...
    adminForm.addEventListener('submit', async function (event) {
        ...
        try {
            ...
            responseMethod.textContent = activeEndpoint.method;
            responsePath.textContent = resolvedPath;
            responsePanelBody.textContent = String(error);
